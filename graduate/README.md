...
В нём:
* Был выполнен анализ данных `Supermarket`
...
Запуск ELT процессов происходит каждый день утром в 05:00 с помощью Apache airlfow

`DAG` этого процесса находится в файле [insert.py](./airflow/dags/insert.py). В этом процессе происходит процесс трансформации данных.

Airflow запускается в контейнере на хосте `localhost`, для используется образ `apache/airflow:2.4.2` c [Dockerhub](https://hub.docker.com/):

```powershell
docker pull apache/airflow:2.4.2
```

На его основе создается свой докер-образ `apache/airflow_dbt:2.4.2` в который доустановим dbt. Описание этого образа находится в файле [Dockerfile](./airflow/Dockerfile)
...
Для работы dbt необходимо предварительно настроить конфигурационный файл [profiles.yml](./airflow/dags/dbt/store/profiles.yml) в котором указываются данные для подключения к БД хранилища. БД хранилища развернута в облаке. 
...
### После запуска `web UI airflow` доступен по адресу http://localhost:8080/ 

В нем активируется DAG `store`:

![DAG store](./data/airflow_dag.jpg)

### После активации DAG `store` он будет итеративно наполнять хранилище данных:
...
# Этап 4. Набор метрик и дашбордов на их основе

Дашборд [Supermarket](https://datalens.yandex/qahn81vmdoeyh) создан при помощи - [DataLens](https://datalens.yandex/).
...
# Этап 5. Документация объектов хранилища данных
...
В работе был использован датасет `Supermarket`. Он был проанализирован и подгружен в БД. Предварительно БД была поднята в облаке. Софрмирована и описана модель данных, на основании которых были созданы `staging`, `dimensioin`, слои данных и таблица фактов по типу схемы звезда. Был настроен ELT-процесс, который инкрементально ежедневно подгружает новые порции данных. Автоматически сформирована документация и опубликована на общедоступном в интеренете ресурсе. Выбраны метрики и на основании них сформирован дашборд с графиками. Дашборд автоматически обновляется и опубликован в интернете.